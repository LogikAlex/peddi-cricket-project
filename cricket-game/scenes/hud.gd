...
@onready var endOverlay: Node2D = $EndScreen
@onready var scoreLabel: Label = $MissedScreen/Score
@onready var endScoreLabel: Label = $EndScreen/Score
...
func showEndOverlay():
	endScoreLabel.text = "SCORE: " + str(Globals.runs)
	var tween = create_tween()
	tween.tween_property(endOverlay, "modulate:a", 1, 1).set_delay(1.5)
	tween.tween_callback(
	func end():
		get_tree().reload_current_scene()
	).set_delay(4.5)
