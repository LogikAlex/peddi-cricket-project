...
var qte_min_size: Vector2 = Vector2(0.01, 0.01)
...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("swing") and !hit:
		...
		tween.set_parallel()
		tween.tween_property(qte_outer_circle, "modulate:a", 0, 0.25).set_delay(0.1)
		tween.tween_property(qte_outer_circle, "scale", qte_outer_circle.scale - Vector2(0.02, 0.02), 0.05)
		tween.tween_property(qte_outer_circle, "scale", qte_outer_circle.scale + Vector2(0.07, 0.07), 0.3).set_delay(0.1)
...
func _start_qte():
	...
	tween.tween_property(qte_outer_circle, "scale", qte_min_size, qte_next_time).set_delay(qte_perfect_time + 0.1)
